...
def compute_residualhealing(tvec, c, unix_tEV):
    """
    return the residual healing before the event: cH(tev - t)
    """
    return [c * resheal_heaviside(t, unix_tEV) for t in tvec]
...
def model_resheal(theta, all=False, **modelparam):
    """
    dv/v base model with residual healing.
    """
    ...
    a0, p1, a_precip, p2, t_shiftdays, S1, log10tmin1, log10tmax1, S2, log10tmin2, log10tmax2, c0, log_f = theta
    ...
    #-------------------------------------------------------------------#
    y_healing = compute_y_healing(unix_tvec[fitting_period_ind], S1, S2, tmin1, tmax1, tmin2, tmax2, unix_tSS, unix_tPF, lib_int)
    resheal = compute_residualhealing(unix_tvec[fitting_period_ind], c0, unix_tSS)

    # Construct model
    model = a0 + p1 * GWL_trim + p2 * T_shift_trim + y_healing + resheal
    #---------------------#

    if all:
        y_healing_SS, y_healing_PF = compute_y_healing_factor(unix_tvec[fitting_period_ind], S1, S2, tmin1, tmax1, tmin2, tmax2, unix_tSS, unix_tPF, lib_int)
        return (model, p1 * GWL_trim, p2 * T_shift_trim, y_healing_SS, y_healing_PF, resheal)
    else:
        return model
...
def log_likelihood(theta, **modelparam):
    ...
    if modelcase.lower() == "base":
        ...
        model = model_resheal(theta, all=False, **modelparam)
...
def log_prior(theta, **modelparam):
    ...
    for i, key in enumerate(keys):
        ...
        if key == "S1": # special case for the boundary
            ...
            # update:2023.5.7 constrain with the coseismic drop Sln(tmax/tmin) to avoid large tradeoff between c and coseismic drop
            if modelparam["modelcase"]=="resheal":
                coseismicdrop_SS = theta[keys.index("S1")]*np.log((10**theta[keys.index("log10tmax1")])/(10**theta[keys.index("log10tmin1")]))
                coseismicdrop_PF = theta[keys.index("S2")]*np.log((10**theta[keys.index("log10tmax2")])/(10**theta[keys.index("log10tmin2")]))
                # print(coseismicdrop_SS, coseismicdrop_PF)
                if (coseismicdrop_SS/coseismicdrop_PF < vmin) or (vmax < coseismicdrop_SS/coseismicdrop_PF):
                    return -np.inf
            else:
