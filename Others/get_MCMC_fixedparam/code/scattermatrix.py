...
def plot_scattermatrix(A, datainds, labels, left=0.1, right=0.9, bottom=0.1, top=0.9, wspace=0.05, hspace=0.05,
    xrange_sigma_factor=3, bincolorgray=0.7, nbin_hist=20, nbin_hist2d=20, Ncontourf=101, Ncontour=7, Ncontour_clip=6,
    ...
    for i in range(Ndim):
        ...
        ncount, bins, patches = ax.hist(data, bins=nbin_hist, range=(xranges[i, 0], xranges[i, 1]), color=[bincolorgray, bincolorgray, bincolorgray],
        ...
        if  isinstance(plot_bestparam, np.ndarray) or isinstance(plot_bestparam, list):
            ax.axvline(plot_bestparam[i], c="k", lw=1.2, ls="-")
    ...
    for i in range(1, Ndim):
        for j in range(0, i):
            ...
            ax.contourf(mX2, mY2, Z2.T, Ncontourf, norm=norm_Z, zorder=4)
            
            # plot contour line
            ax.contour(mX2, mY2, Z2.T, levels_clipped, norm=norm_Z, colors='k', zorder=11, linewidths=0.8)
    ...
    sm = plt.cm.ScalarMappable(cmap='viridis', norm=norm_Z)
